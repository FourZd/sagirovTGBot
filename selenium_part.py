...
class Selenium():
    ...
    def __init__(self):
        # В данном коде будет использоваться Firefox GeckoDriver, т.к. ChromeDriver не считывает русский алфавит #

        self.opts = FirefoxOptions()
        self.opts.add_argument("--headless")
        self.browser = webdriver.Firefox(executable_path=GeckoDriverManager().install(), options=self.opts) 
        self.browser.get('https://b24-iu5stq.bitrix24.site/backend_test/') # get site
    ...
    def fill_third_page(self, birth_date):
        ...
        Select(self.browser.find_element( 
            ...
            )).select_by_value(str(int(birth_date[1]) - 1)) # NINJA CODE ALERT
            # (1) Takes a month from birth_date
            # (2) Convert it into integer
            # (3) Decrease it by 1 so it will be 0-11, not 1-12 format
            # (4) Convert it back into string

        # Select day from cells #
        self.browser.find_element(By.CLASS_NAME, 'vdpCell').find_element(By.XPATH, f"//div[text()='{int(birth_date[2])}']").click()
        self.browser.find_element(
            By.XPATH,
    ...
    def make_screenshot(self, user_id):
        ...
        screenshot_name = os.path.join(SCREENSHOT_DIR, f"{screenshot_time}<{user_id}>.png") # Screenshot dir setting + name join
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                first_name, last_name, email, phone, birth_date, user_id, form_time = request_data # For easier understanding
