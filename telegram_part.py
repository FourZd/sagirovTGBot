...
import datetime # Библиотека для проверки валидности даты
import re # RE для проверки email

import phonenumbers # Библиотека для проверки валидности номера телефона
...
API_TOKEN = '5778947542:AAFbtUyd7BtUbNJRWqdbG0WXw4r3MPLtPk8' # BOT API

logging.basicConfig(level=logging.INFO) # logs

storage = MemoryStorage() # 'local' storage

bot = Bot(token=API_TOKEN) # Bot initialization
dp = Dispatcher(bot, storage=storage) # Dispatcher initializaiton
...
async def pass_phone_number(message: types.Message, state: FSMContext):
    ...
    await FSMData.next()
    await message.reply('И в заключение, напишите вашу дату рождения в виде YYYY-MM-DD')
...
async def pass_birth_date(message: types.Message, state: FSMContext):
    """Form birth date handler"""

    try: # Birth date validation
    ...
    await message.answer('Данные собраны, ожидайте ответа :)')
    await state.finish()
...
def start():
    """Commands on startup"""
    db_init.database_controller.connect_check() # check if DB is connected
    executor.start_polling(dp, skip_updates=True) # Run TG bot
    print('STARTED')
